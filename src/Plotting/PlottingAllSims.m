...
all_T_vals = zeros(n_all_steps,n_node_limits+1);
all_Psi_vals = zeros(n_all_steps,n_node_limits+1);
for node_lim_cnt = 1:n_node_limits
    fprintf("\n--------------------------------------------------\n")
    fprintf("--------------------------------------------------\n")
    ...
    if node_lim_cnt == 1
        all_T_vals(:,1:2) = T_vals;
        all_Psi_vals(:,1:2) = Psi_vals;
    else
        all_T_vals(:,node_lim_cnt+1) = T_vals(:,1);
        all_Psi_vals(:,node_lim_cnt+1) = Psi_vals(:,1);
    end
...
%% Export Results of Simulation Sweep

all_T_vals = all_T_vals*1e-3;
all_Psi_vals = all_Psi_vals*1e-3;

varNames = {'T_inf', 'T_250SDP', 'T_250', 'T_500', 'T_750', 'T_1000', ...
    'T_1500', 'T_2000'};
tab = table( ...
    all_T_vals(:,1), ...
    all_T_vals(:,2), ...
...
writetable(tab, 'torque_sweep.txt','Delimiter',' ');

varNames = {'Psi_inf', 'Psi_250SDP', 'Psi_250', 'Psi_500', 'Psi_750', ...
    'Psi_1000', 'Psi_1500', 'Psi_2000'};
tab = table( ...
    all_Psi_vals(:,1), ...
    all_Psi_vals(:,2), ...
    all_Psi_vals(:,3), ...
    ...
    all_Psi_vals(:,8), ...
    'VariableNames', varNames);
writetable(tab, 'stator_flux_sweep.txt','Delimiter',' ');

% Print final boxplot results
LoadData
