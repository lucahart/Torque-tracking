...
Additionally, we introduce the \textit{\Opt} algorithm. It represents the best-possible choice an algorithm with horizon $N$ could take. It uses a branch-and-bound algorithm with no node limit. Thus, running it until the end gives an optimality guarantee on the solution (Section \ref{sec:4}).
